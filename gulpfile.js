...
gulp.task('serve', function () {
	express()
		...
		.listen(port);
	open('http://localhost:' + port + '/', browswerApp);
});
...
gulp.task('translate', function () {
	return gulp.src('translation/locale-*.json').pipe(angularTranslate()).pipe(insert.prepend('\'use strict\'\;\n')).pipe(replace('"', '\'')).pipe(gulp.dest('app/scripts/translation'));
});
...
gulp.task('ngdocs', [], function () {
  ...
  var options = {
    html5Mode: true,
    startPage: '/api',
    title: "Outfitpic API doc",
    image: "resources/icon.png",
    imageLink: "http://www.outfitpic.com",
    titleLink: "/api"
  }
